...
editHistory:
  - date: '2026-04-17'
    changes: 'Epic 13 UX review (Sally / bmad-agent-ux-designer). Refined `--fetch` output UX: (1) short happy-path summary `fetched N/N repositories` — the common case reads cleanly without `(0 failed, 0 skipped)` noise; (2) expanded summary suppresses zero-valued categories and uses user-facing prose (`1 not a git repo`, `1 no origin remote`) rather than internal status names; (3) pre-fetch anchor line (`fetching N repositories (timeout 60s each, 4 concurrent)...`) prevents buffered-output silence from being read as a hang; (4) timeout failure messages now surface `ASBOX_FETCH_TIMEOUT` hint inline; (5) condensed `--help` text for `--fetch` (full behavioral spec moves to long-form docs); (6) dirty-tree warning block closes with `Ctrl+C now and investigate before the agent starts` (replacing "review before proceeding" — the launch is already rolling, so the phrasing must match actual user agency) and self-advertises `(set ASBOX_SUPPRESS_DIRTY_WARNING=1 to skip this check)` so the escape hatch is discoverable in-message. No new FRs; refinement of FR65 and FR67.'
  - date: '2026-04-17'
    changes: 'Epic 13 stress-test refinement (first-principles + critical-perspective elicitation). Narrowed FR65 (`--fetch`): `git fetch origin` instead of `--all`, per-repo timeout (default 60s, `ASBOX_FETCH_TIMEOUT` override), canonical-path deduplication, explicit non-mutation guarantee (refs/remotes/origin/* and object DB only), buffered per-repo stderr, WARNING-prefixed summary line on partial failure. Strengthened FR64: branch-management guidance must base feature branches on `origin/<default>` (closes the loop with `--fetch`). New FR67: non-fatal dirty-working-tree warning at every launch (suppressible via `ASBOX_SUPPRESS_DIRTY_WARNING=1`) addressing the shared-checkout contamination concern. Updated CLI Interface and `--fetch` flag descriptions. Spawns new Story 13.3; updates 13.1 and 13.2.'
  - date: '2026-04-17'
...
# host_agent_config: true  (enabled by default)
bmad_repos:
...
- `asbox run [agent]` -- Builds if image not present, then launches the sandbox in TTY mode with the configured agent. Override config with `-f`. Override agent with `-a`/`--agent` **or** via positional argument (e.g., `asbox run codex`); specifying both forms is a usage error. `--fetch` runs `git fetch origin` in every configured `bmad_repos` entry (and the project directory mount if it is a git repo) before launching the agent — updating `refs/remotes/origin/*` only, never the working tree. At launch (always, independent of `--fetch`), any mounted git repository with a dirty working tree is surfaced in a non-fatal warning block; suppress with `ASBOX_SUPPRESS_DIRTY_WARNING=1`. The container runs interactively; Ctrl+C stops it.
...
- `--fetch` (on `asbox run`) -- Run `git fetch origin` in every mounted repository before launching the agent, so the agent has up-to-date `refs/remotes/origin/*` even on non-current branches. Updates remote-tracking refs only — working tree, index, HEAD, local branches, and stash are never touched. Bounded concurrency (4 by default); per-repo timeout default 60s, override with `ASBOX_FETCH_TIMEOUT` (surfaced inline in timeout failure messages). Non-git paths and repos without an `origin` remote are skipped and counted under their user-facing labels (`not a git repo`, `no origin remote`). Network failures and timeouts are logged but non-fatal (supports offline use). A pre-fetch anchor line announces the run before per-repo output buffers fill (`fetching N repositories (timeout 60s each, 4 concurrent)...`). The end-of-phase summary is short in the common case (`fetched N/N repositories`) and expands only when there is something to report; on any failure it is prefixed with `WARNING:` and routed to stderr so partial failure is impossible to miss. In `asbox run --help`, the flag is described in a condensed two-line form — full behavioral detail lives in long-form docs
...
- When `--fetch` is passed to `asbox run`: after config parse and mount assembly, and before the `docker run` invocation, the CLI iterates the resolved, deduplicated list of mounted git repositories (project directory mount if a git repo + every `bmad_repos` entry, canonicalized via `filepath.EvalSymlinks`) and runs `git fetch origin` on each — concurrently with a bounded worker pool (default 4) and a per-repo timeout (default 60s, override via `ASBOX_FETCH_TIMEOUT` — the env-var hint is surfaced inline in user-visible timeout failure messages so discovery does not require reading docs) — using the host's existing git credentials (SSH keys, credential helper). The fetch happens on the host, not inside the sandbox, by design: the sandbox has no access to host credentials. Scope is narrow and non-negotiable: only `refs/remotes/origin/*` and the object database are updated; working tree, index, `HEAD`, local branches, stash, and in-progress rebase/merge state are never touched. Before workers dispatch, a pre-fetch anchor line prints to stdout (`fetching N repositories (timeout 60s each, 4 concurrent)...`) so the reader has a landmark during the buffered-output silence of concurrent fetches. Per-repo stderr is buffered and flushed atomically to avoid interleaving. Non-git paths and repos without an `origin` remote are skipped and counted under user-facing labels (`not a git repo`, `no origin remote`). Per-repository failures (network, auth, timeout) are logged but do not abort the launch, so offline use continues to work. The end-of-phase summary uses three shapes: the common case prints a short informational line to stdout (`fetched N/N repositories`); if there were skips but no failures, non-zero skip categories expand inside a parenthetical (`fetched 2/3 repositories (1 not a git repo)`); if any repo failed, the summary is `WARNING:`-prefixed and routed to stderr (`WARNING: fetched 1/3 repositories (2 failed) — see warnings above`). Zero-valued categories are always suppressed so the line stays readable.
- At every `asbox run` invocation (independent of `--fetch`), after mount assembly and after the fetch phase (if any), each mounted git repository's working tree is inspected for dirtiness via `git status --porcelain --ignored=no`. Any dirty repo is named in a non-fatal stderr warning block with the category of change (modified/staged/untracked). This addresses the "shared-checkout contamination" concern: `bmad_repos` paths are not per-session clones, so a prior asbox session or a manual edit elsewhere can leave uncommitted state the agent would silently operate on top of. The warning's closing line tells the user what to actually do (`If unexpected, press Ctrl+C now and investigate before the agent starts.`) — the phrasing matches the user's actual agency, since by the time the warning prints the launch is already rolling toward `docker run`. The warning also self-advertises its suppression mechanism on its final line (`(set ASBOX_SUPPRESS_DIRTY_WARNING=1 to skip this check)`), because an undiscoverable escape hatch is equivalent to no escape hatch. The warning cannot abort launch; set `ASBOX_SUPPRESS_DIRTY_WARNING=1` to skip the check entirely (CI/scripted contexts). Check failures (corrupt repo, missing binary) are logged per-repo and also non-fatal.
...
- FR65: Developer can pass `--fetch` to `asbox run` to execute `git fetch origin` inside each mounted repository (`bmad_repos` entries and the project directory mount when it is a git repository) before the agent is launched. Fetch runs on the host with a bounded concurrency pool (default 4) and a per-repo timeout (default 60s, configurable via `ASBOX_FETCH_TIMEOUT` — the env-var hint appears inline in user-visible timeout failure messages), so the agent sees up-to-date `refs/remotes/origin/*` across all branches — including ones not currently checked out. Deduplication by resolved absolute path ensures the same repo is fetched once even if listed multiple times. Before workers dispatch, a pre-fetch anchor line prints to stdout naming the scope (`fetching N repositories (timeout 60s each, 4 concurrent)...`) so the buffered-output silence of concurrent fetches is not mistaken for a hang. Per-repository failures (network error, auth failure, timeout) are logged as warnings but do not abort the launch (supports offline use); per-repo stderr is buffered and flushed atomically to keep concurrent output legible. Non-git paths and repos without an `origin` remote are skipped and counted under user-facing labels (`not a git repo`, `no origin remote` — never internal status names like `skipped-notgit`). The end-of-phase summary is short in the common case and expands only when there is something to report: `fetched N/N repositories` on the happy path (no parenthetical, stdout); `fetched X/N repositories (1 not a git repo)` when there are non-zero skip categories (zero categories suppressed, stdout); `WARNING: fetched X/N repositories (Y failed) — see warnings above` on any failure (routed to stderr, non-zero skip categories append inside the same parenthetical when present). In `asbox run --help`, the flag description is condensed to ~2 lines; full behavioral detail lives in long-form docs. Scope is intentionally narrow: `--fetch` updates only `refs/remotes/origin/*` and the object database — it never modifies the working tree, index, `HEAD`, local branches, stash, or any in-progress rebase/merge state. Without `--fetch`, no network operation occurs at launch
...
- FR64: The generated `bmad_repos` agent instruction file includes explicit, opinionated branch-management guidance so the agent can manage cross-repository state without trial-and-error: how to create per-task feature branches in each affected repo (branching from `origin/<default>` rather than local default, so that upstream refs fetched host-side by `--fetch` actually translate into up-to-date working state), how to stash or park in-flight work to keep working trees clean between tasks, and how to resume work after a sandbox restart. Guidance is a single convention (not a menu of options) so agent behavior is deterministic across sessions
...
- FR67: At launch, `asbox run` inspects every mounted git repository (`bmad_repos` entries and the project directory mount when it is a git repository) for a dirty working tree and prints a non-fatal warning block to stderr naming each dirty repo and the category of dirtiness (modified, staged, untracked). Files matching `.gitignore` are excluded from the untracked count. The warning block's closing text instructs the user to `press Ctrl+C now and investigate before the agent starts` if the state is unexpected — this phrasing matches the user's actual agency, since by the time the warning prints the launch is already rolling toward `docker run` and "review before proceeding" would misdescribe the option set. The warning block's final line self-advertises the suppression mechanism: `(set ASBOX_SUPPRESS_DIRTY_WARNING=1 to skip this check)` — undiscoverable escape hatches are equivalent to no escape hatch, and users running `asbox run` in CI or hourly cron would otherwise grep the help text for a flag that does not exist. The warning cannot abort launch; it exists to protect the developer against silently operating on top of uncommitted changes left by a prior asbox session or manual edit (since `bmad_repos` paths are shared checkouts, not per-session clones). Check failures (e.g., corrupt repo) are logged per-repo and do not abort launch. Setting `ASBOX_SUPPRESS_DIRTY_WARNING=1` skips the check entirely — intended for CI or scripted contexts where dirty state is expected
